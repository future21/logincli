...
class Pager extends Control
{
     ...
     *
     * This method is used internally to perform all calculations required before the controls to browse the data
     * are displayed.
     *
     * @internal
     */
    ...
    {
            // prevent to execute this control in css and js calls
    ...
    */
    ...
    /**
     * Returns the filename of the CSSFile without extension.
     *
     * For example, for 'style.css', this method would return 'style'.
     *
     * @internal
     */
    function retriveNameFileStyleSheet()
    {
        $keys = explode(".", $this->_cssfile);
        return $keys[0];
    }
...
$css_style_name = $this->retriveNameFileStyleSheet();
echo "div.pagination_{$css_style_name} a, div.pagination_{$css_style_name} span.current_{$css_style_name} \n";
?>
{
...
<?php
	...
	function dumpContents()
	{
        ...
        //Start dumping
        $link_string = '<div class="pagination_'. $css_style_name.'">';

        //Previous button
     	if ($this->current_page > 1) $link_string .= '<a href="' . sprintf($this->baseurl,($this->current_page - 1)) . '" class="prevnext_' .$css_style_name. '">'.$this->_previouscaption.'</a>'."\n";
        else $link_string .= "<a class=\"disabled_{$css_style_name}\">$this->_previouscaption</a>\n";
        ...
        for ($jump_to_page = 1 + (($cur_window_num - 1) * $this->_maxbuttons); ($jump_to_page <= ($cur_window_num * $this->_maxbuttons)) && ($jump_to_page <= $this->number_of_pages); $jump_to_page++)
        {
            ...
            {
          		$link_string .= "<span class=\"current_{$css_style_name}\">$jump_to_page</span>\n";
        	}
            else
            {
        ...
        //Dump next button
      	if (($this->current_page < $this->number_of_pages) && ($this->number_of_pages != 1)) $link_string .= '<a href="' . sprintf($this->baseurl, ($this->current_page + 1)) . '" class="prevnext_'. $css_style_name. '">'.$this->_nextcaption.'</a>'."\n";
        else $link_string .= "<a class=\"disabled_{$css_style_name}\">{$this->_nextcaption}</a>\n";
    ...
    function dumpAdditionalCSS()
    {
        ...
        echo ".pagination_{$css_style_name} {\n";
