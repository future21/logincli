...
class abr_abreviados
{
    ...
    public static function getClientes()
		{
			...
			$obj_cliente->executeQuery("select * from abr_abreviados"); // ejecuta la consulta para traer al cliente

			return $obj_cliente->fetchAll(); // retorna todos los clientes
		}

	function abr_abreviados($nro=0) // declara el constructor, si trae el numero de cliente lo busca , si no, trae todos los clientes
	{
		if ($nro!=0)
		{
			$obj_cliente=new sQuery();
			$result=$obj_cliente->executeQuery("select * from abr_abreviados where id = $nro"); // ejecuta la consulta para traer al cliente
			...
			$this->n_corto=$row['n_corto'];
			$this->n_largo=$row['n_largo'];
	...
	function setn_corto($val)
	 {  $this->n_corto=$val;}
	function setn_largo($val)
	 {  $this->n_largo=$val;}
	...
	private function updateCliente()	// actualiza el cliente cargado en los atributos
	{
            ...
            echo $this->id;
            echo $this->nombre;
			$query="update abr_abreviados set nombre='$this->nombre', apellido='$this->apellido',empresa='$this->empresa',departamento='$this->departamento',n_corto='$this->n_corto',n_largo='$this->n_largo', saldo='$this->saldo' where id = '$this->id'";
	...
	{
			$obj_cliente=new sQuery();
			$query="insert into abr_abreviados ( nombre, apellido, empresa,departamento,n_corto,n_largo, importe) values ('$this->nombre', '$this->apellido','$this->empresa','$this->departamento','$this->n_corto','$this->n_largo', '$this->saldo')";
	...
	{
			$obj_cliente=new sQuery();
			$query="delete from abr_abreviados where id=$this->id";
...
class abr_usuarios
{
    ...
    public static function getClientes()
		{
			$obj_cliente=new sQuery();
			$obj_cliente->executeQuery("select * from abr_usuarios"); // ejecuta la consulta para traer al cliente

			return $obj_cliente->fetchAll(); // retorna todos los clientes
		}

	function abr_usuarios($nro=0) // declara el constructor, si trae el numero de cliente lo busca , si no, trae todos los clientes
	{
		if ($nro!=0)
		{
			$obj_cliente=new sQuery();
			$result=$obj_cliente->executeQuery("select * from abr_usuarios where id = $nro"); // ejecuta la consulta para traer al cliente
			$row=mysql_fetch_array($result);
			$this->id=$row['id'];
			$this->nombre=$row['nombre'];
			$this->apellido=$row['apellido'];
            ...
            $this->Old_password=$row['encrypted_password'];
			$this->salt=$row['salt'];
	...
	function getEmail()
	 { return $this->email;}
	function getEncrypted_password()
	 { return $this->encrypted_password;}
	function getOld_password()
	...
	function setEmail($val)
	 {  $this->email=$val;}
	function setEncrypted_password($val)
	 {  $this->encrypted_password=$val;}
	function setOld_password($val)
	...
	private function updateCliente()	// actualiza el cliente cargado en los atributos
	{
			...
			if (isUserExisted($this->email)) {
				...
				$hash = checkhashSSHA($salt, $this->encrypted_password);
				if ($this->getOld_password() == $this->encrypted_password) {
			        $query="update abr_usuarios set nombre='$this->nombre', apellido='$this->apellido',email='$this->email',adm='$this->adm' where id = '$this->id'";
				} else {
					$hash = hashSSHA($this->encrypted_password);
					$encrypted_password = $hash["encrypted"]; // encrypted password
					$salt = $hash["salt"]; // salt
			        $query="update abr_usuarios set nombre='$this->nombre', apellido='$this->apellido',email='$this->email',encrypted_password='$encrypted_password',salt='$salt',adm='$adm' where id = '$this->id'";
                }
	...
	private function insertCliente()	// inserta el cliente cargado en los atributos
	{
			...
			$obj_cliente=new sQuery();
			$query="insert into abr_usuarios ( nombre, apellido, email,encrypted_password,salt,created_at, adm) values ('$this->nombre', '$this->apellido','$this->email','$encrypted_password','$salt',NOW(), '$this->adm')";
			send_mail($email,$subject,$message,$headers);
	...
	{
			$obj_cliente=new sQuery();
			$query="delete from abr_usuarios where id=$this->id";
			$obj_cliente->executeQuery($query); // ejecuta la consulta para  borrar el cliente
			return $obj_cliente->getAffect(); // retorna todos los registros afectados
