...
class Calendar
{
  ...
  var $sfondo_mese="Red"; //background of the month at the top
  var $testo_mese="White"; //color of the text of Month
  var $sfondo_settimana="Orange"; //background of the week at the top
  ...
  function Calendar($lang)
  {
    switch ($lang)
    {
      case "it":
        ...
        break;
      case "es":
        $this->day_lang="'l','m','m','j','v','s','d'";
        $this->month_lang="'enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'";
        $this->short_month_lang="'ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic'";
        break;
      case "en":
	...
	function CreateCalendar($date_initial,$name,$link_back,$link_next,$date_format="2")
  {
    ...
    //date_format=2 then format date is dd/mm/yyyy
		...
		CalendarPopup_Up_DisplayMonthYear('calendario".$num."_div', 'calendario".$num."_monthYear', 'calendario".$num."_Up_PreDisplayCalendar', 'calendario".$num."_Up_PreMonthYear', monthnames, theDate, 'Aplicar', 'Cancelar', $link_back, $link_next);
	    }
